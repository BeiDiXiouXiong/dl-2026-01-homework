...
## 一、实验背景
在使用 Conda 创建 Python 3.11 环境后，执行以下命令安装依赖时，出现 `ResolutionImpossible` 依赖解析失败错误，说明 `requirements_broken.txt` 中存在多处依赖冲突问题：
```bash
pip install -r requirements_broken.txt
```
二、问题定位与分析
本次共定位出 5 类典型依赖问题，按冲突影响优先级排序如下：
问题 1：numba 与 numpy 版本硬冲突（依赖版本约束冲突）
现象：
报错信息：
```text
numba 0.56.4 depends on numpy <1.24 and >=1.18
but requirements.txt specifies numpy==1.26.4
```
原因分析：
